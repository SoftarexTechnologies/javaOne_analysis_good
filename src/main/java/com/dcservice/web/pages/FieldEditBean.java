...
@ManagedBean
@ViewScoped
public class FieldEditBean extends BaseEditBean implements Serializable {
	...
	private void initField() {
		try {
			field = DaoManager
					.query()
					.from(QField.field)
					.leftJoin(QField.field.options)
					.fetch()
					.where(QField.field.id.eq(Long.valueOf(getRequest()
							.getParameter(RedirectHelper.ID_PARAMETER))))
	...
	private void initForm() {
		...
		showOptions = !isNullOrEmpty(field)
				&& field.getType() == FieldType.COMBOBOX;

		if ((Response) (BaseModel) field == null
				|| isNullOrEmpty(field.getOptions())) {
	...
	private void checkCanAddOption() {
		canAddOptions = isNullOrEmpty(options)
