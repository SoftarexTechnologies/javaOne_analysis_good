...
public abstract class BaseValidationBean extends BasePageBean {
	...
	public void addFieldExeption(UIComponent component, String message) {
		...
		ValidatorHelper.markNotValid(component, isNullOrEmpty(msg) ? message
				: msg, getContext());
	...
	public void addRequiredFieldExeption(UIComponent component) {
		...
		try {
			...
			try {
				method = component.getClass().getMethod(LABEL_METHOD,
						new Class[0]);
			...
			if (method == null) {
				try {
					method = component.getClass().getDeclaredMethod(
							LABEL_METHOD, new Class[0]);
				} catch (NoSuchMethodException e) {
					log.error(e.getMessage(), e);
				}
			}
			...
			if (method != null) {
				...
				if (!isNullOrEmpty(label)) {
					String msg = String.format("%s %s", label,
							ResourcesHelper.getString("isRequiredField"));
	...
	public void markInvalid(UIComponent component, String message) {
		...
		ValidatorHelper.markNotValid(component,
				ResourcesHelper.getString(message), getContext());
	}
